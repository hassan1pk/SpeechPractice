...
{
    static class NormalPractice
    {
        static int repeatTimes = 3;

        public static void GenerateAudio()
        {
            string inputFile = "Practice1.txt"; // Input text file
            string tempWavFile = "Practice1.wav"; // Temporary WAV file
            string finalMp3File = "Practice1.mp3"; // Final MP3 output file
            ...
            using (var synthesizer = new SpeechSynthesizer())
            {
                ...
                using (var writer = new WaveFileWriter(outputWavFile, new WaveFormat(24000, 16, 1)))
                {
                    foreach (var line in File.ReadLines(inputFile))
                    {
                        ...
                        for (int i = 0; i < repeatTimes; i++) // Repeat each line 3 times
                        {
